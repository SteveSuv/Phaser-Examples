class Example extends Phaser.Scene {
  ...
  preload() {
    this.load.atlas(
      "banner",
      "assets/atlas/banners.png",
      "assets/atlas/banners.json",
    );
  }
  ...
  createBanner(key, flag, runeY = 100) {
    ...
    banner.stamp("banner", `Banner_${GetRandom(crests)}`, 256, 256, {
      alpha: 0.3,
    ...
    banner.stamp("banner", `Badges_${GetRandom(runes)}`, 256, runeY, {
      scale: 0.5,
      originX: 1,
    });
    banner.stamp("banner", `Badges_${GetRandom(runes)}`, 256, runeY, {
