...
let text = [
    ...
    [
      ...
      colrP,
      0,
      "Douglas Adams and Rodney Matthews.    Now a little comment about ripping...",
    ],
    ...
    [
      ...
      colrP,
      0,
      "All our code is copyrighted and may not be re-used in any program or modified in any way whatsoever.",
    ],
    [
      fNoop,
      colrB,
      0,
      "It is also illegal to distribute it under any form other than that in which it was released:",
    ],
    ...
    [
      ...
      colrY,
      0,
      "If you wonder why we have such a strict policy, it is because some commercial lamers helped themselves to parts of our code, as well as inumerable groups.",
    ],
    [
      fSin1,
      colrP,
      0,
      "Some hints about the 4-bit hardware scroller coming up...    In January 1991, I (Alien) got my 4-bit hardware-scroller to work.",
    ],
    [
      fSin2,
      colrP,
      0,
      "It allows you to move the whole screen left and right by increments of 4 pixels. The source has been published in my series of articles about overscan in ST Magazine, a French publication.",
    ],
    [
      fTria,
      colrY,
      0,
      "As this technique is brand-new, it may not work on some Atari ST's. If it doesn't work on yours, please contact us.",
    ],
    [
      fBce2,
      colrB,
      0,
      "Time to grab a pen..........    To obtain 4 bit hardware scrolling on an ST, you have to switch to midrez after the passage to hirez used to free the left border, and then wait 0,4,8,12 cycles before switching back to lowrez.",
    ],
    [
      fNoop,
      colrP,
      0,
      "This affects the way the shifter works, and delays the displaying of the picture by a multiple of 500 nanoseconds, which results in an effective shift in the picture...",
    ],
    [
      fSin2,
      colrY,
      0,
      "Note the method described here does not work on some STE's.   Time is up, so let's cut the crap:",
    ],
    [
      fRot1,
      colrB,
      0,
      "According to Vickers of Legacy sitting next to us, you've been reading this text for over an hour...",
    ],
    [
      fCyld,
      colrP,
      0,
      "But the original version was over 10 kb long, ensuring 4 hours of pleasant reading!",
    ],
    [
      fGrow,
      colrY,
      0,
      "See you in the next St Connexion Production, scheduled to be released within the next 2 years!                                 ",
    ],
  ],
  frame,
  letters,
...
class Example extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.atlas(
      "bobs",
      "assets/tests/twist/bobs.png",
      "assets/tests/twist/bobs.json",
    );
  }
      ...
      for (let y = 0; y < 16; y++) {
        if (col[y] === 1) {
          switch (callback) {
            // Sine
            case fSin1:
              posy = mid + 30 * Math.sin((x + iteration) / 8) + (y - 8) * 10;
              break;
            case fSin2:
              posy = mid + 30 * Math.sin((x + iteration) / 6) + (y - 8) * 10;
              break;
            ...
            case fBce1:
              posy =
                mid +
                20 -
                40 * Math.abs(Math.sin((x + iteration) / 8.75)) +
                (y - 8) * 10;
              break;
            case fBce2:
              posy =
                mid +
                20 -
                40 * Math.abs(Math.sin((x + iteration) / 17.5)) +
                (y - 8) * 10;
              break;

            // Rotate
            case fRot1:
              posy = mid + (y - 8) * 10 * Math.sin((x + iteration) / 8);
              break;
            case fRot2:
              posy = mid + (y - 8) * 10 * Math.sin((x + iteration) / 5);
              break;

            // Cylinder
            case fCyld:
              posy = mid + 100 * Math.sin((x + y + iteration) / 8.75);
              break;
            ...
            case fTria:
              posy =
                mid +
                10 * Math.abs((((x + iteration) / 8) % 12) - 6) -
                30 +
                (y - 8) * 10;
              break;
            ...
            // Pack/Unpack
            case fGrow:
              posy = mid + (y - 8) * (10 + 3 * Math.sin((x + iteration) / 10));
              break;
