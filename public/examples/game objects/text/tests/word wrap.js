class Example extends Phaser.Scene {
  ...
  create() {
    ...
    // --- DEFAULTS ---
    this.assert(
      "Word wrap width should be null",
      text.style.wordWrapWidth === null,
    );
    this.assert(
      "Advanced word wrap should be false",
      text.style.wordWrapUseAdvanced === false,
    );
    this.assert(
      "Word wrap callback should be null",
      text.style.wordWrapCallback === null,
    );
    this.assert(
      "Word wrap callback scope should be null",
      text.style.wordWrapCallbackScope === null,
    );
    this.assert(
      "Wrapped text should be one line",
      text.getWrappedText().length === 1,
    );

    {
      ...
      this.assert(
        "First line not have whitespace collapsed",
        lines[0].includes("The   sky"),
      );
...

    {
      ...
      this.assert(
        "First line have whitespace collapsed",
        lines[0].includes("The sky"),
      );
      text.setWordWrapWidth(null);
    }

    {
      ...
      text.setWordWrapCallback(
        function (string, textObject) {
          this.assert(
            "Second argument should be the text gameobject",
            text === textObject,
          );
          this.assert(
            "Scope should match the given scope object",
      ...
      let lines = text.getWrappedText();
      this.assert(
        "Wrapped text should be exactly two lines",
        lines.length === 2,
      );
      this.assert(
        'Wrapped text should be ["1", "2"]',
        lines[0] === "1" && lines[1] === "2",
      );
