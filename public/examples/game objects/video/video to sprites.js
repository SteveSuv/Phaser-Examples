class Example extends Phaser.Scene {
  ...
  buildScene() {
    ...
    this.bufferA = this.add
      .shader("Fire Buffer A", 0, 0, 1024, 1024)
      .setRenderToTexture();

    //  The Fire Buffer B shader.
    this.bufferB = this.add
      .shader("Fire Buffer B", 0, 0, 1024, 1024)
      .setRenderToTexture();

    //  The final shader. It will render to a texture called 'FireShader'
    this.shader = this.add
      .shader("Fire", 0, 0, 1024, 1024)
      .setRenderToTexture("FireShader", true);

    //  Hook the sampler2D uniforms up for the multi-pass:

    this.bufferA.setSampler2DBuffer(
      "iChannel0",
      this.rt.getWebGLTexture(),
      1024,
      1024,
      0,
    );
    this.bufferA.setSampler2DBuffer(
      "iChannel1",
      this.bufferB.glTexture,
      1024,
      1024,
      1,
    );

    this.bufferB.setSampler2DBuffer(
      "iChannel0",
      this.shader.glTexture,
      1024,
      1024,
      0,
    );
