...
class Example extends Phaser.Scene {
  ...
  init() {
    ...
    this.add
      .text(10, 10, 'Press "space" to fire bullet', {
        font: "16px Courier",
        fill: "#ffffff",
      })
      .setDepth(100);
  ...
  preload() {
    ...
    this.load.atlas(
      ...
      "/particles/flares.json",
    );
  }

  create() {
    // Just stars background
    const bg = this.add.image(0, 0, "bg").setOrigin(0, 0).setTint(0x333333);

    const planet = this.physics.add
      .image(this.sys.scale.width - 100, this.sys.scale.height / 2, "planet")
      .setScale(0.2);
    planet.flipX = true;
    ...
    this.ship = this.add
      .image(100, this.sys.scale.height / 2, "ship")
      .setDepth(2);
