class Example extends Phaser.Scene {
  ...
  update() {
    const distance = Phaser.Math.Distance.BetweenPoints(
      this.source.body.center,
      this.target,
    );

    this.distanceText.setText(
      `Distance: ${distance.toFixed(3)} Speed: ${this.source.body.speed.toFixed(3)}`,
    );
    ...
    if (this.source.body.speed > 0) {
      ...
      this.source.body.velocity.scale(Phaser.Math.SmoothStep(distance, 0, 20));
