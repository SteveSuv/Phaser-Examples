class Example extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("cat", "assets/animations/nyan/cat.png", {
      frameWidth: 97,
      frameHeight: 59,
    });
  ...
  update() {
    if (this.cat) {
      this.cat.x =
        this.cat.width / 2 +
        (this.catAstroPhi.seek / this.catAstroPhi.duration) *
          (800 - this.cat.width);
  ...
  setup() {
    ...
    sm.add(this.catAstroPhi, "seek", 0, this.catAstroPhi.duration)
      .step(0.01)
      .listen();
    ...
    rainbow.mask = new Phaser.Display.Masks.GeometryMask(
      this,
      this.rainbowMask,
    );
    ...
    this.input.on("drag", (pointer, cat, dragX, dragY) => {
      ...
      this.catAstroPhi.seek =
        ((cat.x - cat.width / 2) / (800 - cat.width)) *
        this.catAstroPhi.duration;
    });
  }
}
