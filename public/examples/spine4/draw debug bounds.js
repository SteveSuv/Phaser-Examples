...
function preload() {
  ...
  this.load.spine(
    "set1",
    "demos.json",
    ["atlas1.atlas", "atlas2.atlas", "heroes.atlas"],
    true,
  );
}
...
function create() {
  ...
  this.createToggle(
    ...
    true,
    this.spine.setDebugRegionAttachments.bind(this.spine),
  );
  this.createToggle(
    10,
    60,
    "Draw Bounding Boxes",
    true,
    this.spine.setDebugBoundingBoxes.bind(this.spine),
  );
  this.createToggle(
    10,
    85,
    "Draw Mesh Hull",
    true,
    this.spine.setDebugMeshHull.bind(this.spine),
  );
  this.createToggle(
    10,
    110,
    "Draw Mesh Triangles",
    true,
    this.spine.setDebugMeshTriangles.bind(this.spine),
  );
  this.createToggle(
    10,
    135,
    "Draw Paths",
    true,
    this.spine.setDebugPaths.bind(this.spine),
  );
  this.createToggle(
    10,
    160,
    "Draw Skeleton XY",
    false,
    this.spine.setDebugSkeletonXY.bind(this.spine),
  );
  this.createToggle(
    10,
    185,
    "Draw Clipping",
    true,
    this.spine.setDebugClipping.bind(this.spine),
  );
...
function createToggle(x, y, label, enabled, callback) {
  var button = this.add
    .image(x, y, enabled ? "on" : "off")
    .setOrigin(0)
    .setScrollFactor(0);

  var text = this.add
    .text(x + 90, y + 6, label, { font: "16px Courier", fill: "#ffffff" })
    .setShadow(1, 1)
    .setScrollFactor(0);
