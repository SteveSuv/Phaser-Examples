class Example extends Phaser.Scene {
  ...
  preload() {
    this.load.tilemapTiledJSON(
      "this.map",
      "assets/tilemaps/maps/matter-destroy-tile-bodies.json",
    );
    this.load.image(
      "platformer_tiles",
      "assets/tilemaps/tiles/platformer_tiles.png",
    );
    this.load.spritesheet("player", "assets/sprites/dude-cropped.png", {
      frameWidth: 32,
      frameHeight: 42,
    });
  }
  ...
  create() {
    ...
    const bgLayer = this.map
      .createLayer("Background Layer", tileset, 0, 0)
      .setScale(this.mapScale);
    const groundLayer = this.map
      .createLayer("Ground Layer", tileset, 0, 0)
      .setScale(this.mapScale);
    const fgLayer = this.map
    ...
    this.playerController.sensors.bottom = M.Bodies.rectangle(sx, h, sx, 5, {
      isSensor: true,
    });
    ...
    this.matter.world.on("collisionstart", (event) => {
      for (let i = 0; i < event.pairs.length; i++) {
        ...
        if (
          (bodyA === playerBody && bodyB.label === "disappearingPlatform") ||
          (bodyB === playerBody && bodyA.label === "disappearingPlatform")
        ) {
          const tileBody =
            bodyA.label === "disappearingPlatform" ? bodyA : bodyB;
    ...
    this.matter.world.on("afterupdate", (event) => {
      this.playerController.blocked.right =
        this.playerController.numTouching.right > 0 ? true : false;
      this.playerController.blocked.left =
        this.playerController.numTouching.left > 0 ? true : false;
      this.playerController.blocked.bottom =
        this.playerController.numTouching.bottom > 0 ? true : false;
    });
  ...
  update(time, delta) {
    ...
    if (
      this.cursors.up.isDown & canJump &&
      this.playerController.blocked.bottom
    ) {
