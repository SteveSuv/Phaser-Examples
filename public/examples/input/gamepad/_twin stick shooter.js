class Example extends Phaser.Scene {
  ...
  create() {
    ...
    this.text = this.add.text(10, 10, "Press a button on the Gamepad to use", {
      font: "16px Courier",
      fill: "#00ff00",
    });
    ...
    this.input.gamepad.on(
      ...
      (pad, button, index) => {
        if (pad.getAxisTotal() < 4) {
          this.text.setText(
            "Gamepad does not have enough axis for a twin-stick demo",
          );
        } else {
...
class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  launch() {
    ...
    const p = Phaser.Geom.Rectangle.RandomOutside(
      Enemy.spaceOuter,
      Enemy.spaceInner,
    );
