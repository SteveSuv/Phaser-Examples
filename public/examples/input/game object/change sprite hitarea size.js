class Example extends Phaser.Scene {
  ...
  create() {
    ...
    const text = this.add.text(
      10,
      10,
      "Click to toggle rotation\nMouse over the hit areas",
      { font: "16px Courier", fill: "#00ff00" },
    );

    this.atari1.on("pointerover", () => {
      text.setText("Over Image 1");
    });
    this.atari1.on("pointerout", () => {
      text.setText("");
    });

    this.atari2.on("pointerover", () => {
      text.setText("Over Image 2");
    });
    this.atari2.on("pointerout", () => {
      text.setText("");
    });

    this.atari3.on("pointerover", () => {
      text.setText("Over Image 3");
    });
    this.atari3.on("pointerout", () => {
      text.setText("");
    });

    //  Draw the hit areas to a graphics object so we can visualize it:
    this.hitarea1 = this.add.rectangle(
      this.atari1.x,
      this.atari1.y,
      this.atari1.width,
      this.atari1.height,
    ...
    this.hitarea2 = this.add.rectangle(
      this.atari3.x,
      this.atari3.y,
      this.atari3.width + 60,
      this.atari3.height + 60,
      0xff00ff,
      0.5,
    );
