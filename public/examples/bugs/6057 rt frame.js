...
class Example extends Phaser.Scene {
  preload() {
    ...
    this.load.spritesheet("marioss", "assets/bugs/mario4x4.png", {
      frameWidth: 128,
      frameHeight: 128,
    });
  }
  ...
  create() {
    const origin = this.add
      .renderTexture(0, 0, 256, 256)
      .setIsSpriteTexture(true);
    ...
    this.add
      .text(0, sqSize * 2, "Original (RT)", { color: "#000", fontSize: 24 })
      .setOrigin(0, 0);
    ...
    // `Image`
    this.add
      .text(offset, sqSize * 2, "Image", { color: "#000", fontSize: 24 })
      .setOrigin(0, 0);
    [
      ["square-1", 0, 0],
      ["square-2", sqSize + 1, 0],
    ...
    // `Sprite`
    this.add
      .text(offset, sqSize * 2, "Sprite", { color: "#000", fontSize: 24 })
      .setOrigin(0, 0);
    [
      ["square-1", 0, 0],
      ["square-2", sqSize + 1, 0],
    ...
    // `Blitter`
    this.add
      .text(offset, sqSize * 2, "Blitter", { color: "#000", fontSize: 24 })
      .setOrigin(0, 0);
    ...
    // `RenderTexture`
    this.add
      .text(offset, sqSize * 2, "RT.drawFrame", { color: "#000", fontSize: 24 })
      .setOrigin(0, 0);
    const target = this.add
      .renderTexture(offset, 0, 512, 512)
      .setIsSpriteTexture(true);
    [
      ["square-1", 0, 0],
      ["square-2", sqSize + 1, 0],
