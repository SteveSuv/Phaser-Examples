class Example extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = 0; i < 12; i++) {
      this.timerEvents.push(
        this.time.addEvent({ delay: 200 + i * 400, loop: true }),
      );
    }
  ...
  drawClock(x, y, progress) {
    ...
    const dest = Phaser.Math.RotateAroundDistance(
      { x: x, y: y },
      x,
      y,
    ...
    const p1 = Phaser.Math.RotateAroundDistance(
      { x: x, y: y },
      x,
      y,
    ...
    const p2 = Phaser.Math.RotateAroundDistance(
      { x: x, y: y },
      x,
      y,
      Phaser.Math.DegToRad(angle + 5),
